...
class IPFencingService:
    ...
    def update_ip_reputation(
        ...
        reputation.reputation_score = (
            reputation.threat_intel_score * weights['threat_intel'] +
            reputation.historical_score * weights['historical'] +
            reputation.recent_activity_score * weights['recent_activity'] +
            reputation.geo_score * weights['geo']
        )
    ...
    def increment_ip_stats(self, ip: str, was_blocked: bool = False, is_anomaly: bool = False, is_threat: bool = False):
        ...
        if not reputation:
            reputation = IPReputation(ip=ip, first_seen=datetime.utcnow())
            self.db.add(reputation)
        
        reputation.total_requests += 1
        if was_blocked:
            reputation.blocked_requests += 1
        if is_anomaly:
            reputation.anomaly_count += 1
        if is_threat:
